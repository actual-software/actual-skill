  ...
  0)
    ...
    if [ "${trimmed#\{}" != "$trimmed" ] && [ "${trimmed%\}}" != "$trimmed" ]; then
      if has_unicode_escape "$trimmed" || has_duplicate_permission_decision "$trimmed"; then
        # A \uXXXX escape anywhere, or a second literal permissionDecision key,
        # means none of the literal-bytes matches below can be trusted: either
        # can ride alongside a literal "deny" (decoding, on duplicate-key
        # last-wins, to allow) just as easily as hide beside a systemMessage.
        # So this check gates BOTH branches, not just the notice one -- refuse
        # to forward the verdict in any shape. Say so rather than dropping it
        ...
        emit_pretooluse_notice \
          "Actual plan governance received a verdict it could not safely interpret (it contained an escaped character sequence or a duplicate permissionDecision key); this plan was not checked against .actual/rules/."
